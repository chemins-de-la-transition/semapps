...
const sortAndFilterOnDate = (ids, eventsData, nb) => {
  ...
  sortedIds = sortedIds.filter((elemId) => {
    return (
      typeof eventsData[elemId]['pair:startDate'] !== 'undefined' && eventsData[elemId]['pair:startDate'].length !== 0
    );
  });
  ...
  if (futureIds.length < nb) {
    ...
    sortedIds = futureIds.concat(
      pastIds.sort((firstElemId, SecondElemId) => {
        return sortStartDate(firstElemId, SecondElemId, 'DESC', eventsData);
      })
    );
